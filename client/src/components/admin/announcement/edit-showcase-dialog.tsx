...
interface EditShowcaseDialogProps {
  open: boolean;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        toast.success(response.data.message || "Showcase updated successfully");
        onOpenChange(false);
        onSuccess?.();
      } else {
        toast.error(response.data.message || "Failed to update showcase");
      }
    } catch (error: any) {
      toast.error(
        error.response?.data?.message || "Failed to update showcase"
      );
    } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh]">
        <DialogHeader>
          <DialogTitle>Edit Department Showcase</DialogTitle>
          <DialogDescription>
            Update the showcase details
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-6 px-4">
            ...
            <div className="flex justify-end gap-2 pt-4 border-t">
              ...
              <Button type="submit" disabled={isSubmitting}>
                {isSubmitting ? "Updating..." : "Update Showcase"}
              </Button>
            </div>
          </form>
        </ScrollArea>
